# 智能抠图整合工具 / Smart Cutout Tool

[简体中文](README.md) | [English](README.en.md)

面向 Windows 的本地 AI 抠图桌面工具。应用在本机处理图片，支持单图、批量抠图、前后效果对比、边缘优化、自动保存，以及 CPU / 可选 CUDA 推理。应用首次打开时可选择简体中文或 English，之后可在设置中随时切换。
...
- CPU 基础版可直接运行；选择 CUDA 后可按提示自动下载，或手动下载两个指定 wheel 并由应用安装独立 GPU 运行时
...
重新运行新版安装程序时，会自动识别旧安装并在原位置升级；模型权重、CUDA 运行时、插件、配置、日志和输出均会保留。升级过程中不可更改原安装位置；如需迁移位置，请先备份上述目录后再进行全新安装。
